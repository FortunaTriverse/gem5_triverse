...
// 根据预取信息计算预取地址
void
OptPrefetcher::calculatePrefetch(const PrefetchInfo &pfi, std::vector<AddrPriority> &addresses)
{
    if (!pfi.hasPC() || pfi.isWrite()) return;
    Addr pc = pfi.getPC();
    ...
    if (opt_entry) {
        ...
        for (int i = 0; i < OptLines - 1; i++) {
            if (opt_entry->hist[i + OptLines - 1].rawCounter() >= con_opt) {
                ...
                bool send_r = sendPFWithFilter(pfi, pf_tgt_addr, addresses,0);
...
// 发送预取请求并进行过滤
bool
OptPrefetcher::sendPFWithFilter(const PrefetchInfo &pfi, Addr addr, std::vector<AddrPriority> &addresses,unsigned delay)
{
