...
#ifndef __MEM_CACHE_PREFETCH_Triverse_HH__
#define __MEM_CACHE_PREFETCH_Triverse_HH__
...
#include "params/TriverseHashedSetAssociative.hh"
...
struct TriversePrefetcherParams;
 ...
 */
class TriverseHashedSetAssociative : public SetAssociative
{
  public:
  ...
  public:
    ...
    TriverseHashedSetAssociative(
        const TriverseHashedSetAssociativeParams &p)
      : SetAssociative(p), ways(0),max_ways(8)
    {
    }
    ~TriverseHashedSetAssociative() = default;
};



class Triverse : public Queued
{
    ...
    const int stream_degree;
    const double replace_threshold;
    const double prefetch_threshold;
    const double increase_stride_threshold;
    const double decrease_stride_threshold;
    const int max_stride;
    const int max_degree;
    const int min_degree;
    const int dynamic_stride_degree;
    int real_degree;
    
    double global_accuracy_degree_coefficient;
    const bool monitor_accuracy;
	  const double global_accuracy_increase_threshold;
	  const double global_accuracy_decrease_threshold;
	  const double update_accuracy_threshold;
	  const double high_accuracy_degree_coefficient;
	  const double low_accuracy_degree_coefficient;
	  const double normal_accuracy_degree_coefficient;
    ...
    struct StrideEntry : public TaggedEntry
    {
        int stride_1;
        int stride_2;
        SatCounter8 confidence_1;
        SatCounter8 confidence_2;
        int degree;

        StrideEntry() : stride_1(0), stride_2(0), confidence_1(4), confidence_2(4),degree(4)
    ...
    AssociativeSet<StrideEntry> strideTable;
...
assert(idx>=0);
  ...
  public:
    Triverse(const TriversePrefetcherParams &p);
    ~Triverse() = default;
    ...
    void stridePrefetch(Addr pf_addr,int new_stride, 
    ...
    void streamPrefetch(Addr pf_addr, std::vector<AddrPriority> &addresses,
...
#endif // __MEM_CACHE_PREFETCH_Triverse_HH__
